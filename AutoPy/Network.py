...
def model_fn(features, mode):
    ...
    output = tf.layers.dense(inputs = dropOut, units = OUTPUT_SIZE, activation = tf.identity)
...
totalLoss = tf.reduce_sum(loss) * next_element["reward"]#tf.multiply(tf.reduce_sum(loss),tf.maximum( next_element["reward"],tf.multiply(next_element["reward"],0.1)))
