...
def model_fn(features, mode):
    ...
    dense2 = tf.layers.dense(inputs = input_layer[:,-1], units = 200, activation = tf.nn.leaky_relu)
...
feedHolder = tf.placeholder(tf.float32, [None,SEQ_LEN,INPUT_SIZE])
...
totalLoss = tf.reduce_mean(loss) * (next_element["reward"] - avg)#tf.multiply(tf.reduce_sum(loss),tf.maximum( next_element["reward"],tf.multiply(next_element["reward"],0.1)))
